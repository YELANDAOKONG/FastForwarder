...
public class TcpMiddleman : IDisposable
{
    ...
    private ITcpMiddleman _middleman;
    ...
    )
    {
        ...
        _middleman = middleman ?? new NoneTcpMiddleman();
    ...
    private async Task WorkingThreadLocalToRemote(Socket localSocket, Socket remoteSocket, int random = -1, CancellationToken token = default)
    {
        ...
        try
        {
            while (_isWorking && !IsDisposed && !_cancellationTokenSource.IsCancellationRequested)
            {
                ...
                try
                {
                    var data = await _middleman.DataAsync(
                        (IPEndPoint)localSocket.RemoteEndPoint!,
                        (IPEndPoint)remoteSocket.RemoteEndPoint!,
                        random,
                        packetCounter,
                        true,
                        new ReadOnlyMemory<byte>(buffer, 0, bytesRead).ToArray()
                    );
                    packetCounter++;
                    await remoteSocket.SendAsync(new ReadOnlyMemory<byte>(data, 0, data.Length), SocketFlags.None, token).ConfigureAwait(false);
        ...
        try
        {
            while (_isWorking && localSocket.Connected && remoteSocket.Connected && _isWorking && !IsDisposed)
            {
                ...
                try
                {
                    ...
                    packetCounter++;
                    await localSocket.SendAsync(new ReadOnlyMemory<byte>(data, 0, data.Length), SocketFlags.None, token).ConfigureAwait(false);
